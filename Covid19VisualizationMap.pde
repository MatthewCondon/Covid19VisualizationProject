...
void setup() {
  ...
  for (int i = 0; i < allStates.size()-1; i++) {
    if (allStates.get(i).getDate().equals(allStates.get(i+1).getDate()) == false) allDates.add(allStates.get(i).getDate());
  }
  print(allDates);
...
void getData(String d) {
  for (int i = 0; i < allStates.size(); i++) {
    if (allStates.get(i).getDate().equals(d)) {
      ...
      if (allStates.get(i).getName().equals("New Hampshire")) NewHampshire = allStates.get(i).count();
      if (allStates.get(i).getName().equals("New Jersey")) NewJersey = allStates.get(i).count();
      if (allStates.get(i).getName().equals("New Mexico")) NewMexico = allStates.get(i).count();
      if (allStates.get(i).getName().equals("New York")) NewYork = allStates.get(i).count();
      if (allStates.get(i).getName().equals("North Carolina")) NorthCarolina = allStates.get(i).count();
      if (allStates.get(i).getName().equals("North Dakota")) NorthDakota = allStates.get(i).count();
      ...
      if (allStates.get(i).getName().equals("Rhode Island")) RhodeIsland = allStates.get(i).count();
      if (allStates.get(i).getName().equals("South Carolina")) SouthCarolina = allStates.get(i).count();
      if (allStates.get(i).getName().equals("South Dakota")) SouthDakota = allStates.get(i).count();
      ...
      if (allStates.get(i).getName().equals("West Virginia")) WestVirginia = allStates.get(i).count();
...
void draw() {
  ...
  textSize(10);
  text("Data as per The New York Times Dev Portal\nand reported by the states. Depicts positive\ncases reported per day. To make changes\nautomatically, simply input 'all'.", 610, 550);
  fill(255);
  textSize(20);
  text("Key:", 500, 530);
  textSize(10);
  ...
  if (save.equals("all")) {
    ...
    //getData("2020-01-21");
    // Grabs data and displays by user
    textSize(25);
    text(save, 50, 450);
    getData(save);
  }
...
void next() {
  place++;
  if (place >= allDates.size()) place = 0;
}
